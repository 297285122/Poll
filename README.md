...
注：邮箱验证信息存储在redis中，有效期为1天。此接口并不同步发送验证邮件，只将用户信息写入MQ即返回，由邮件发送worker来处理邮件发送。
...
|userId|Array|Y|用户ID|'5caee5599d1a9c2280b3b283'|-|
...
描述: 创建投票活动。
注：考虑到候选人信息多，将候选人信息分步存储。创建投票活动前，请先创建候选人信息。
...
|candidates|Object|Y|候选人信息集合|-|候选人数不少于4个不多于10个|
|candidateId|String|Y|候选人ID|'2008450740201411110000174436'|-|
...
描述: 查看投票活动详细情况，包含投票结果信息。
 注：投票结果理解为候选人的得票数。投票活动信息会被缓存至redis中。目前投票结果与投票活动信息嵌套存储。投票期间，投票结果从redis中读取，投票详情数据更新至mongo后，从mongo中读取。（需求中未具体要求查看投票活动信息）
...
####  [返回接口目录](#main)
 ...
 描述: 用户提交投票。
 注：相同ip地址或邮箱地址视为同一用户。投票期间，用户投票的统计信息保存在redis中，投票详情写入MQ（可考虑将详情保存至redis并从redis更新到mongo中）。后续由投票worker将数据存储到mongo中。
